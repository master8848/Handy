...
use crate::{DownloadProgress, ModelManager};
use anyhow::Result;
...
/// from the UI (the production wrapper maps them onto core events) so the
/// transport logic is testable without an event sink.
...
impl ModelManager {
    ...
    /// [`Self::download_http_resumable_with_events`] wired to the core event
    /// sink — the production entry point.
        ...
        Self::download_http_resumable_with_events(
            ...
            &move |event| match event {
                HttpDownloadEvent::Progress(progress) => {
                    events.on_event(CoreEvent::ModelDownloadProgress {
                ...
                HttpDownloadEvent::VerificationStarted => {
                    events.on_event(CoreEvent::ModelVerificationStarted {
                        model_id: id.clone(),
                    })
                }
                HttpDownloadEvent::VerificationCompleted => {
                    events.on_event(CoreEvent::ModelVerificationComplete {
                        model_id: id.clone(),
                    })
                }
            },
    ...
    /// verification notifications as a callback instead of touching the event
    /// sink, so the failure-mode behavior below is exercised by tests against a
    /// local socket server.
    ///
