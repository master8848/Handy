...
use crate::model::{EngineType, ModelManager};
use crate::settings::{
    get_settings, AppSettings, OrtAcceleratorSetting, SettingsStore, TranscribeAcceleratorSetting,
};
use crate::{apply_custom_words, filter_transcription_output};
use anyhow::Result;
...
pub struct TranscriptionManager {
    ...
    /// UI event sink (replaces Handy's app handle).
...
impl TranscriptionManager {
    pub fn new(
        ...
        let watcher_store = settings_store.clone();
        let manager = Self {
        ...
        // Start the idle watcher
        {
            ...
            let handle = thread::spawn(move || {
                ...
                while !shutdown_signal.load(Ordering::Relaxed) {
                    ...
                    let settings = get_settings(&*watcher_store);
                    let timeout = settings.model_unload_timeout;

                    // handy_core has no shared audio-manager registry, so
                    // recording state is not observable here; the recording
                    // manager touches activity itself during sessions.
                    ...
                    if let Some(limit_seconds) =
                        crate::settings::model_unload_timeout_seconds(timeout)
                    {
    ...
    pub fn unload_model(&self) -> Result<()> {
        ...
        // Emit unloaded event
        self.events.on_event(CoreEvent::ModelStateChanged {
    ...
    pub fn maybe_unload_immediately(&self, context: &str) {
        // handy_core's ModelUnloadTimeout has no "immediately" variant; the
        // idle watcher handles the timeout-based unload instead.
        debug!("Immediate unload requested after {context} (ignored; no immediate-unload setting)");
    }
    ...
    pub fn load_model_with_device(
        ...
        apply_accelerator_settings(&*self.settings_store);
        ...
        let loaded_engine = match model_info.engine_type {
            EngineType::TranscribeCpp => {
                ...
                let (backend, gpu_device) = match device_index {
                    ...
                    None => {
                        let settings = get_settings(&*self.settings_store);
    ...
    pub fn initiate_model_load(&self) {
        ...
        *is_loading = true;
        ...
        thread::spawn(move || {
            ...
            let settings = get_settings(&*self_clone.settings_store);
    ...
    pub fn emit_stream_working(&self, kind: StreamWorkKind) {
        debug!("Stream working phase: {:?}", kind);
        self.events.on_event(CoreEvent::StreamPhase {
            phase: "working".to_string(),
        });
    }

    fn emit_stream_text(&self, committed: &str, tentative: &str) {
        self.events.on_event(CoreEvent::StreamText {
...
fn effective_language_for_model(
    ...
    match model_manager.get_model_info(model_id) {
        Some(info) => crate::model::effective_language(
...
fn select_transcribe_backend(setting: TranscribeAcceleratorSetting) -> Backend {
    ...
    match setting {
        ...
        | TranscribeAcceleratorSetting::Ort
        | TranscribeAcceleratorSetting::Cpp => Backend::Auto,
    }
}
...
fn resolve_gpu_device(_setting: TranscribeAcceleratorSetting, gpu_device: i32) -> i32 {
    if transcribe_gpu_disabled_for_host() || gpu_device <= 0 {
        return 0;
    }
...
pub fn apply_accelerator_settings(store: &dyn SettingsStore) {
    ...
    let ort_pref = match settings.ort_accelerator {
        ...
        OrtAcceleratorSetting::Cpu | OrtAcceleratorSetting::CpuMemOptimize => {
            accel::OrtAccelerator::CpuOnly
        }
        OrtAcceleratorSetting::Cuda | OrtAcceleratorSetting::CudaTensorrt => {
            accel::OrtAccelerator::Cuda
        }
...
fn available_transcribe_accelerators(gpu_disabled: bool) -> Vec<String> {
    if gpu_disabled {
        ...
        vec!["auto".to_string(), "cpp".to_string(), "ort".to_string()]
    }
}
...
mod tests {
    ...
    fn normal_hosts_preserve_every_transcribe_accelerator_setting() {
        for setting in [
            ...
            assert_eq!(
                select_transcribe_backend(setting),
                if transcribe_gpu_disabled_for_host() {
                    Backend::Cpu
                } else {
                    Backend::Auto
    ...
    fn emulated_x64_on_arm64_forces_every_transcribe_setting_to_cpu() {
        for setting in [
            ...
            if transcribe_gpu_disabled_for_host() {
                assert_eq!(select_transcribe_backend(setting), Backend::Cpu);
            } else {
                assert_eq!(select_transcribe_backend(setting), Backend::Auto);
            }
        }
        assert_eq!(available_transcribe_accelerators(true), ["auto"]);
