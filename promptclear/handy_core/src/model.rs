// Ported from Handy (https://github.com/cjpais/Handy) — MIT license.
// Copyright (c) 2025 CJ Pais. All rights reserved.
use crate::event::{CoreEvent, EventSink};
use crate::model_capabilities::{
    CapabilityProbe, CapabilityProber, Compatibility, GgufHeaderProber,
};
use crate::settings::{get_settings, write_settings, AppSettings, SettingsStore};
use anyhow::Result;
...
use crate::download::{HttpDownloadOutcome, DOWNLOAD_STALL_TIMEOUT};
...
pub enum ModelSource {
    ...
    Local,
    /// A user-registered local model at an arbitrary filesystem path
...
/// Bridges hf-hub's async download progress to the `model-download-progress`
...
impl HfDownloadProgress {
    fn new(events: Arc<dyn EventSink>, model_id: String) -> Self {
        Self {
            events,
            model_id,
    ...
    fn emit(&self, downloaded: u64, total: u64) {
        ...
        self.events.on_event(CoreEvent::ModelDownloadProgress {
...
pub struct ModelManager {
    ...
    /// Cached copy of the persisted settings, refreshed by [`Self::register_custom_model`].
...
impl ModelManager {
    pub fn new(
        ...
        let settings = Arc::new(Mutex::new(get_settings(&*settings_store)));
    ...
    pub fn rescan_local_models(&self) -> Result<()> {
        ...
        self.events.on_event(CoreEvent::ModelsUpdated);
        Ok(())
    }
    ...
    fn migrate_bundled_models(&self) -> Result<()> {
        ...
        let bundled_models_dir = crate::paths::default_models_dir();

        for filename in &bundled_models {
            let bundled_path = bundled_models_dir.join(filename);
    ...
    fn migrate_gigaam_to_directory(&self) -> Result<()> {
        ...
        // The vocab ships as a bundled resource; in the UI-free crate there is
        // no resource bundle, so fall back to the models dir and skip the
        // migration gracefully when it isn't there.
        let vocab_path = crate::paths::default_models_dir().join("gigaam_vocab.txt");
        if !vocab_path.exists() {
            warn!(
                "Bundled GigaAM vocab not found at {:?}; skipping migration",
                vocab_path
            );
            return Ok(());
        }
    ...
    fn update_download_status(&self) -> Result<()> {
        ...
        let custom_model_path = get_settings(&*self.settings_store).custom_model_path;
        let mut models = self.available_models.lock().unwrap();
    ...
    fn auto_select_model_if_needed(&self) -> Result<()> {
        let mut settings = get_settings(&*self.settings_store);
        ...
        if !settings.selected_model.is_empty() {
            ...
            if !exists {
                ...
                write_settings(&*self.settings_store, &settings)?;
            }
        }
        ...
        if settings.selected_model.is_empty() {
                ...
                .find(|model| model.is_downloaded)
            {
                ...
                write_settings(&*self.settings_store, &updated_settings)?;
    ...
    async fn download_hf_model(
            ...
            || self.models_dir.join(&filename).exists()
        {
            self.update_download_status()?;
            self.events
                .on_event(CoreEvent::ModelDownloadComplete { model_id });
            return Ok(());
        }
        ...
        let hf_error = loop {
            ...
            let progress = HfDownloadProgress::new(self.events.clone(), model_id.clone());
        ...
        self.events.on_event(CoreEvent::ModelDownloadComplete {
            model_id: model_id.clone(),
        });
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<()> {
        ...
        if model_info.is_directory {
            ...
            self.events.on_event(CoreEvent::ModelExtractionStarted {
                model_id: model_id.to_string(),
            });
            ...
            archive.unpack(&temp_extract_dir).map_err(|e| {
                ...
                self.events.on_event(CoreEvent::ModelExtractionFailed {
                    model_id: model_id.to_string(),
            ...
            self.events.on_event(CoreEvent::ModelExtractionComplete {
                model_id: model_id.to_string(),
            });
    ...
    pub fn delete_model(&self, model_id: &str) -> Result<()> {
        ...
        if let ModelSource::HuggingFace { repo_id, revision } = &model_info.source {
            ...
            self.update_download_status()?;
            self.events.on_event(CoreEvent::ModelDeleted {
                model_id: model_id.to_string(),
            });
            return Ok(());
        }
    ...
    pub fn get_model_path(&self, model_id: &str) -> Result<PathBuf> {
        ...
        if let ModelSource::LocalPath = &model_info.source {
            let custom_path = get_settings(&*self.settings_store)
                .custom_model_path
                .ok_or_else(|| anyhow::anyhow!("No custom model path set"))?;
    ...
    /// Register a user-provided local model file (PromptClear extension). The
    /// model resolves directly from `path` via `AppSettings::custom_model_path`
    /// and is never downloaded or cached by Handy.
    /// Synchronous wrapper around `download_model` for UI threads that
    /// don't manage a tokio runtime: runs the download to completion on a
    /// short-lived runtime.
    pub fn download_model_sync(&self, model_id: &str) -> Result<()> {
        let runtime = tokio::runtime::Runtime::new()?;
        runtime.block_on(self.download_model(model_id))
    }

    pub fn register_custom_model(&self, path: PathBuf) -> Result<()> {
        if !path.is_file() {
            return Err(anyhow::anyhow!("Custom model file not found: {:?}", path));
        }
        ...
        let engine_type = match path.extension().and_then(|e| e.to_str()) {
            Some(ext) if ext.eq_ignore_ascii_case("gguf") => EngineType::TranscribeCpp,
            _ => EngineType::Parakeet,
        };

        let info = ModelInfo {
            id: "custom-model".to_string(),
            name,
            description: "User-provided local model".to_string(),
            filename: path
                .file_name()
                .and_then(|s| s.to_str())
                .unwrap_or_default()
        ...
        self.available_models
            .lock()
            .unwrap()
            .insert("custom-model".to_string(), info);

        let mut settings = get_settings(&*self.settings_store);
        settings.custom_model_path = Some(path.to_string_lossy().to_string());
        write_settings(&*self.settings_store, &settings)?;
        {
            let mut cached = self.settings.lock().unwrap();
            *cached = settings;
        }
    ...
    /// Remove the registered local model and clear `custom_model_path`.
    pub fn unregister_custom_model(&self) {
        self.available_models.lock().unwrap().remove("custom-model");
        let mut settings = get_settings(&*self.settings_store);
        settings.custom_model_path = None;
        let _ = write_settings(&*self.settings_store, &settings);
        {
            let mut cached = self.settings.lock().unwrap();
            *cached = settings;
        }
    ...
    pub fn cancel_download(&self, model_id: &str) -> Result<()> {
        ...
        self.events.on_event(CoreEvent::ModelDownloadCancelled {
            model_id: model_id.to_string(),
        });
