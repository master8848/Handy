...
use egui_shadcn::{
    Badge, BadgeVariant, Button, ButtonVariant, Card, ComponentSize, Kbd, Label, LucideIcon,
    Progress, SelectValue, Separator, ShadcnThemeExt, Slider, StatusBar, Switch, Tabs, Textarea,
    Toolbar, Tooltip,
};
...
fn model_choice(selected_id: &str, models: &[handy_core::ModelInfo]) -> ModelChoice {
    models
        ...
        .map(|model| ModelChoice {
            id: model.id.clone(),
            label: format!("{} ({} MB)", model.name, model.size_mb),
        })
...
pub fn editor(ui: &mut egui::Ui, app: &mut PromptClearApp) {
    ...
    Card::new().show(ui, |ui| {
        ...
        let response = if app.app_settings.spellcheck_enabled {
...
pub fn status_bar(ui: &mut egui::Ui, app: &mut PromptClearApp) {
    ...
    StatusBar::new().show(ui, |ui| {
        ...
        Separator::vertical().show(ui);
        Badge::new(if app.app_settings.spellcheck_enabled {
            "Spell: on"
        } else {
            "Spell: off"
        })
